...
int check_rightangle_bend()
{
    ...
    else
    {
        ...
        if(line>=10&&line<=55&&Right_line[line]>=94&&Left_line[line+15]==0)
        {
            count = 0;
        ...
        else if((line>=55&&Right_line[line]>=100)&&Left_line[line+5]==0)
        {
            count = 0;
            i = line - 5;
            for(j = 0;j<188;j++)
...
//        }
//    }
    if(Abs(CX, 94)<=30&&three_flag==0)//&&num>=15
    {
...
void road_type()
{
     ...
     else if(entered_Cross==0&&check_cross_road_out(num_white,white_line[num_white-1]))  //检测是否到达十字
     {
         flag=2;//十字直道标志位
         Cross_status=1;//进入十字状态
     }

     else if((right = check_rightangle_bend())&&rightangle_flag!=2)//判断直角弯（判断到后将rightangle加1）
     {
         if(right==2)//快到直角弯
         {
             flag=9;
