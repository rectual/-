...
int check_cross_road_out(int num1,int b)   //判断十字
{
    ...
    if(num1>=20)//中间出现20行白行
    {
        ...
        else
        {
            ...
            if(Abs(L_U_dot,R_U_dot)<=20&&L_U_dot>=10&&R_U_dot>=10)
            {
...
void Repair_Line()
 {
    ...
    if(white_line[0]>=115)
    {
        for(row=119;row>=10;row--)
        {
            if((Left_line[row-1]-Left_line[row])>5)
            {
                L_D_dot=row;
                break;
            }
        ...
        for (row =119; row >= 10; row--)
        {
            if((Right_line[row-1]-Right_line[row])<-5)
            {
                R_D_dot=row;
                break;
            }
        }
        k=(double)(Left_line[L_U_dot]-Left_line[L_D_dot])/(L_D_dot-L_U_dot);
        for(row=L_D_dot-1;row<=L_U_dot;row--)
        {
            Left_line[row]=Left_line[L_U_dot]+(int)((L_U_dot-row)*k);
            Pixel[row][Left_line[row]] = 0;
        }
        for(row=R_D_dot-1;row<=L_U_dot;row--)
        {
            Right_line[row]=Right_line[R_U_dot]-(int)((R_U_dot-row)*k);
            Pixel[row][Right_line[row]] = 0;//显示补的线
        }
        for(row=L_D_dot+5;row>=min(L_U_dot, R_U_dot);row--)
        {
...
//十字区
   if(Cross_status>0)   //Cross_status=0不在  Cross_status=1 发现十字   Cross_status=2 完全进入十字   Cross_status=3 再发现十字
   {
       ...
       if(Cross_status==1&&repair_status==1&&num_white<=5)  //完全进入 结束补线
       {
           repair_status=0;
           Cross_status++;
       }
